...
function App() {
  ...
  const [hasData, setHasData] = useState(false) // Tracking if analysis has run successfully
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      // 3. Clean Stringified JSON formatting safely
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-gradient-to-br from-[#0f172a] via-[#111827] to-[#1e1b4b] text-white font-sans antialiased">
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-10 lg:p-14 space-y-6 pb-24 lg:pb-14">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-white/5 pb-4">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Upload metrics to align with industry compliance schemas.</p>
          </div>
          {hasData && (
            <button
              onClick={handleReset}
              className="mt-2 md:mt-0 flex items-center justify-center gap-1.5 bg-white/10 hover:bg-white/20 px-4 py-2 rounded-xl text-xs font-bold transition-all"
            >
        ...
        {/* CONDITIONALLY RENDER FORM: Hides form layout once data arrives on mobile to guarantee output visibility */}
        {!hasData ? (
          <section className="bg-white/5 border border-white/10 rounded-2xl p-4 md:p-8 backdrop-blur-md shadow-2xl">
            <form onSubmit={handleUpload} className="space-y-4">
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                ...
                <div className="flex flex-col">
                  ...
                  <textarea
                    ...
                    placeholder="Paste the target description details..."
              ...
              <div className="flex justify-end">
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-gradient-to-r from-cyan-500 via-blue-500 to-purple-500 text-black py-3 rounded-xl font-extrabold text-xs tracking-wide shadow-xl flex items-center justify-center gap-2"
                >
                  {loading ? (
                    <>
          ...
          /* OUTPUT TELETEMETRY DISPLAY PANELS */
          <section className="mt-2 animate-fadeIn">
            {activeTab === "dashboard" && (
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="bg-white/5 border border-white/10 rounded-2xl p-6 flex flex-col items-center justify-center min-h-[130px] shadow-lg">
              ...
              <div className="bg-white/5 border border-white/10 rounded-2xl p-5 space-y-3 shadow-lg">
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {skills.map((skill, index) => (
                    <span key={index} className="bg-cyan-500/10 border border-cyan-500/20 text-cyan-300 px-3 py-1.5 rounded-lg text-[11px] font-bold uppercase">
                      {skill}
                    </span>
                  ))}
                </div>
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-slate-950/90 border-t border-white/10 backdrop-blur-lg flex justify-around p-2 z-50">
        {[
